...
export function IncomeForm({ projectId, setOpen }: IncomeFormProps) {
  ...
  const submitAction = async (formData: FormData) => {
    ...
    const { error } = await createProjectIncomeAction(formData);
    if (error) {
      toast.error(error);
    } else {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="mt-2">
          <input
            ...
            onChange={() => setErrors({ ...errors, source: '' })}
          />
        </div>
        <div className="h-[20px]">
      ...
      <div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
        <FormButton text="Add Income" pendingText='Adding Income...' />
        <button
          type="button"
